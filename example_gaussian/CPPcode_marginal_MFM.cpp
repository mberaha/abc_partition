...
void update_cluster_allocation(vec Y,
  ...
  for(uword i = 0; i < clust.n_elem; i++){
    ...
    for(arma::uword j = 0; j < k; j++){
      probs(j)  = log(accu(clust == j) + gamma)
      - 0.5 * log(variances(j) * 2 * M_PI) - 0.5 * pow(Y(i) - means(j), 2) / variances(j);
    }
    probs(k) = log(Vnt_MAR(k)) + log(gamma) + 
...
Rcpp::List main_univ_MFM(vec Y,
  ...
  vec clust(Y.n_elem);
  vec means(Y.n_elem);
  vec variances(Y.n_elem);
  
  clust = initialize_part(Y.n_elem);
  means.fill(0.0);
  variances.fill(1.0);
  para_clean_univ(means,variances, clust);
  ...
  for(uword iter = 0; iter < niter; iter++){
    ...
    update_cluster_allocation(Y, clust, means, variances, m0, k0, a0, b0, gamma, Vnt_MAR);
    accelerate_MFM(Y, means, variances, clust, m0, k0, a0, b0);
  ...
  time = double(current_s-start_s)/CLOCKS_PER_SEC;
